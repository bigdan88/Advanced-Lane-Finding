...
class tracker():
    ...
    def __init__(self, Mywindow_width, Mywindow_height, Mymargin, My_ym = 1, My_xm = 1, Mysmooth_factor = 15):
        ...
        #the window pixel width of the center values, used to count pixels insude center windows to determine curve values
        self.window_width = Mywindow_width
        
        #the window pixel of the center values, used to count pixels inside center widnwos to determine curve values
        ...
        self.ym_per_pix = My_ym #meters per pixel in vertial axis
    ...
    def find_windows_centroids(self, warped):
        ...
        for level in range(1,(int)(warped.shape[0]/window_height)):
            ...
            #fidn the best left centroiid by using past left center as a reference
            #use window_width/2 as offset because convolution signal reference is at right side of window, not center of window
            offset = window_width/2
            l_min_index = int(max(l_center + offset - margin, 0))
            l_max_index = int(max(l_center + offset + margin, warped.shape[1]))
            l_center = np.argmax(conv_signal[l_min_index: l_max_index])+ l_min_index - offset
            #find the best right centroid by using [ast right center as a reference
